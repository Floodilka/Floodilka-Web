...
# Скрипт для переноса floodilka на новый сервер
# Запускать на СТАРОМ сервере (Digital Ocean)

set -e

echo "🚀 Начинаем миграцию floodilka на новый сервер..."
...
mongodump --db floodilka --out ./backup/mongodb
...
tar -czf floodilka-migration-backup.tar.gz \
    backup/ \
    deployment/ \
...
echo "✅ Backup создан: floodilka-migration-backup.tar.gz"

# Копирование на новый сервер
echo "🚚 Копирование данных на новый сервер..."
scp floodilka-migration-backup.tar.gz $NEW_SERVER_USER@$NEW_SERVER_IP:/tmp/
...
echo "   tar -xzf /tmp/floodilka-migration-backup.tar.gz"
echo ""
echo "3. Восстановите MongoDB:"
echo "   mongorestore --db floodilka backup/mongodb/floodilka"
echo ""
...
echo "   nano /etc/nginx/sites-available/floodilka"
echo "   # Замените server_name на ваш новый IP/домен"
echo ""
echo "7. Перезапустите сервисы:"
echo "   sudo systemctl restart nginx"
echo "   sudo -u floodilka pm2 restart floodilka-backend"
echo ""
...
# Инструкция по настройке floodilka на новом сервере

## 1. Подключение к серверу
ssh $NEW_SERVER_USER@$NEW_SERVER_IP

## 2. Установка базового ПО
sudo bash /var/www/floodilka/deployment/setup.sh

## 3. Установка MongoDB
sudo bash /var/www/floodilka/deployment/setup-mongodb.sh

## 4. Распаковка данных
cd /var/www/floodilka
tar -xzf /tmp/floodilka-migration-backup.tar.gz

## 5. Восстановление данных
mongorestore --db floodilka backup/mongodb/floodilka
cp -r backup/uploads/* backend/uploads/ 2>/dev/null || true
cp backup/.env backend/.env

## 6. Настройка production
sudo bash /var/www/floodilka/deployment/setup-production.sh

## 7. Обновление nginx конфигурации
sudo cp /var/www/floodilka/deployment/nginx-http.conf /etc/nginx/sites-available/floodilka
# Отредактируйте server_name в файле:
sudo nano /etc/nginx/sites-available/floodilka

## 8. Активация nginx
sudo ln -sf /etc/nginx/sites-available/floodilka /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl restart nginx

## 9. Деплой приложения
sudo bash /var/www/floodilka/deployment/update.sh
