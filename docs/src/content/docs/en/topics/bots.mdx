...
```bash
curl https://api.floodilka.com/v1/channels/CHANNEL_ID/messages \
  -X POST \
...
```bash
curl https://api.floodilka.com/v1/guilds/GUILD_ID/members?limit=100 \
  -H "Authorization: Bot YOUR_BOT_TOKEN"
```
	</TabItem>
...
For anything reactive (respond to messages, track member joins, handle voice state), connect to the [Gateway](/gateway/overview/). Identify with your bot token and the `intents` bitfield that declares which event categories you want.
