...
pub fn load_config() -> Result(Config, String) {
  use api_endpoint_public_raw <- result.try(required_env("FLOODILKA_API_PUBLIC_ENDPOINT"))
  let api_endpoint_raw = case optional_env("FLOODILKA_ADMIN_API_ENDPOINT") {
    option.Some(v) -> v
    option.None -> api_endpoint_public_raw
  }
  use media_endpoint_raw <- result.try(required_env("FLOODILKA_MEDIA_ENDPOINT"))
  use cdn_endpoint_raw <- result.try(required_env("FLOODILKA_CDN_ENDPOINT"))
  use admin_endpoint_raw <- result.try(required_env("FLOODILKA_ADMIN_ENDPOINT"))
  use web_app_endpoint_raw <- result.try(required_env("FLOODILKA_APP_ENDPOINT"))
  ...
  use base_path_raw <- result.try(required_env("FLOODILKA_PATH_ADMIN"))
  use port <- result.try(required_int_env("FLOODILKA_ADMIN_PORT"))
  ...
  let metrics_endpoint = case optional_env("FLOODILKA_METRICS_HOST") {
