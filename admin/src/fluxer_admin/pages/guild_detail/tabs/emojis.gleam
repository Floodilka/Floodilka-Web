...
fn render_emojis(
  ...
  ui.card(ui.PaddingMedium, [
    ui.heading_card_with_margin(
      "Эмодзи (" <> int.to_string(list.length(emojis)) <> ")",
    ),
    case list.is_empty(emojis) {
      True ->
        h.p([a.class("text-sm text-neutral-600")], [
          element.text("Эмодзи не найдены для этого сервера."),
        ])
      False ->
        h.div(
    ...
    [
      ...
      h.div([a.class("px-4 py-3 flex-1 flex flex-col")], [
        ...
        h.a(
          ...
          [
            element.text("Загрузил: " <> emoji.creator_id),
          ],
        ),
        h.form(
          [
          ...
          [
            ...
            ui.button("Удалить эмодзи", "submit", ui.Danger, ui.Small, ui.Full, [
              a.class("mt-2"),
            ]),
          ],
        ),
...
fn render_permission_notice() {
  ui.card(ui.PaddingMedium, [
    ...
    h.p([a.class("text-sm text-neutral-600")], [
      element.text("Для управления эмодзи сервера необходим ACL asset:purge."),
    ]),
  ])
}
