    ...
    h.div(
      ...
      [
        h.p([a.class("text-yellow-800")], [
          element.text(
            "Сервис метрик не настроен. Установите FLUXER_METRICS_HOST для активации.",
          ),
        ]),
      ],
    ),
  ...
  h.div([], [
    ...
    h.div([a.class("mt-6")], [
      h.div(
        ...
        [
          h.div([a.class("p-6")], [
            ui.heading_section("Обзор очередей"),
            ui.text_small_muted("Текущие итоги очередей задач по всем воркерам"),
            h.div([a.id("queue-stats-container"), a.class("mt-4")], [
              h.div(
                ...
                [
                  render_loading_stat_card("Всего в ожидании", "pending-count"),
                  render_loading_stat_card("Всего выполняется", "running-count"),
                  render_loading_stat_card("Всего ошибок", "failed-count"),
                  render_loading_stat_card(
                    "Загрузка воркеров",
                    "concurrency-utilization",
                  ),
                  render_loading_stat_card("Среднее время ожидания", "avg-wait-time"),
                ],
              ),
            ]),
          ]),
    ...
    h.div([a.class("mt-6")], [
      h.div(
        ...
        [
          h.div([a.class("p-6")], [
            ui.heading_section("Размеры очередей Redis"),
            ui.text_small_muted("Очереди фоновых задач в Redis"),
            h.div(
              [
    ...
    h.div([a.class("mt-6")], [
      h.div(
        ...
        [
          h.div([a.class("p-6")], [
            ui.heading_section("Разбивка по задачам"),
            ui.text_small_muted(
              "Ожидание, выполнение, процент успеха и повторы по задачам",
            ),
    ...
    h.div([a.class("mt-6")], [
      h.div(
        ...
        [
          h.div([a.class("p-6")], [
            ui.heading_section("Пропускная способность задач"),
            ui.text_small_muted(
              "Задач завершено в минуту (успешно, ошибки, повторы)",
            ),
            h.div([a.class("mt-4")], [
              element.element(
                "canvas",
    ...
    h.div([a.class("mt-6")], [
      h.div(
        ...
        [
          h.div([a.class("p-6")], [
            ui.heading_section("Глубина очередей во времени"),
            ui.text_small_muted("Количество ожидающих и выполняющихся задач во времени"),
            h.div([a.class("mt-4")], [
              element.element(
                "canvas",
    ...
    h.div([a.class("mt-6")], [
      h.div(
        ...
        [
          h.div([a.class("p-6")], [
            ui.heading_section("Перцентили задержки задач"),
            ui.text_small_muted("Распределение времени обработки (p50, p90, p99)"),
            h.div([a.class("mt-4")], [
              element.element(
                "canvas",
    ...
    h.div([a.class("mt-6")], [
      h.div(
        ...
        [
          h.div([a.class("p-6")], [
            ui.heading_section("Статус cron-задач"),
            ui.text_small_muted("Отслеживание выполнения запланированных задач"),
    ...
    h.div([a.class("mt-6")], [
      h.div(
        ...
        [
          h.div([a.class("p-6")], [
            ui.heading_section("Недавние ошибки задач"),
            ui.text_small_muted("Задачи с недавними сбоями"),
...
fn render_jobs_script(metrics_endpoint: String) -> String {
  ...
  (async function() {
    ...
    try {
      ...
      if (taskSet.size === 0) {
        container.innerHTML = '<div class=\"text-neutral-500 text-sm\">Нет данных по задачам</div>';
      } else {
        ...
        html += '<th class=\"px-4 py-2 text-left text-xs font-medium text-neutral-500 uppercase\">Задача</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Ожидание</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Выполнение</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Успешно</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Ошибки</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Процент успеха</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Повторы</th>';
      ...
      document.getElementById('per-task-container').innerHTML = '<div class=\"text-red-500 text-sm\">Не удалось загрузить данные по задачам</div>';
    }
      ...
      if (timestamps.length > 0) {
        new Chart(document.getElementById('jobThroughputChart'), {
          ...
          data: {
            ...
            datasets: [
              { label: 'Успешно', data: alignData(successResp.data, timestamps), borderColor: 'rgb(34, 197, 94)', backgroundColor: 'rgba(34, 197, 94, 0.1)', fill: true, tension: 0.1, spanGaps: true },
              { label: 'Ошибки', data: alignData(errorResp.data, timestamps), borderColor: 'rgb(239, 68, 68)', backgroundColor: 'rgba(239, 68, 68, 0.1)', fill: true, tension: 0.1, spanGaps: true },
              { label: 'Повторы', data: alignData(retryResp.data || [], timestamps), borderColor: 'rgb(249, 115, 22)', backgroundColor: 'rgba(249, 115, 22, 0.1)', fill: true, tension: 0.1, spanGaps: true }
            ]
          },
          options: {
            responsive: true,
      ...
      if (timestamps.length > 0) {
        new Chart(document.getElementById('queueDepthsChart'), {
          ...
          data: {
            labels: timestamps.map(formatTimeLabel),
            datasets: [
              { label: 'Ожидание', data: alignData(pendingResp.data, timestamps), borderColor: 'rgb(59, 130, 246)', backgroundColor: 'rgba(59, 130, 246, 0.1)', fill: true, tension: 0.1, spanGaps: true },
              { label: 'Выполнение', data: alignData(runningResp.data, timestamps), borderColor: 'rgb(168, 85, 247)', backgroundColor: 'rgba(168, 85, 247, 0.1)', fill: true, tension: 0.1, spanGaps: true }
            ]
          },
          options: {
            responsive: true,
    ...
    try {
      ...
      if (Object.keys(percentiles).length > 0) {
        ...
        document.getElementById('latencyPercentilesChart').parentElement.innerHTML = '<div class=\"text-neutral-500 text-sm text-center py-8\">Нет данных о задержке</div>';
      }
    } catch (e) {
    ...
    try {
      ...
      if (cronBreakdown.length === 0) {
        container.innerHTML = '<div class=\"text-neutral-500 text-sm\">Нет данных о cron-задачах</div>';
      } else {
        let html = '<div class=\"overflow-x-auto\"><table class=\"min-w-full divide-y divide-neutral-200\">';
        html += '<thead class=\"bg-neutral-50\"><tr>';
        html += '<th class=\"px-4 py-2 text-left text-xs font-medium text-neutral-500 uppercase\">Cron-задача</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Последний запуск</th>';
        html += '<th class=\"px-4 py-2 text-center text-xs font-medium text-neutral-500 uppercase\">Статус</th>';
        html += '</tr></thead><tbody class=\"bg-white divide-y divide-neutral-200\">';
        ...
        for (const cron of cronBreakdown.sort((a, b) => a.label.localeCompare(b.label))) {
          ...
          const statusText = isOverdue ? 'Просрочено' : 'OK';

          html += '<tr class=\"hover:bg-neutral-50\">';
          html += '<td class=\"px-4 py-2 text-sm font-medium text-neutral-900\">' + cron.label + '</td>';
          html += '<td class=\"px-4 py-2 text-sm text-neutral-600 text-right\">' + formatMs(cron.value) + ' назад</td>';
      ...
      document.getElementById('cron-status-container').innerHTML = '<div class=\"text-red-500 text-sm\">Не удалось загрузить данные cron</div>';
    }
    ...
    try {
      ...
      if (tasksWithErrors.length === 0) {
        container.innerHTML = '<div class=\"text-green-600 text-sm flex items-center gap-2\"><svg xmlns=\"http://www.w3.org/2000/svg\" class=\"h-5 w-5\" viewBox=\"0 0 20 20\" fill=\"currentColor\"><path fill-rule=\"evenodd\" d=\"M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z\" clip-rule=\"evenodd\" /></svg> Нет недавних ошибок</div>';
      } else {
        let html = '<div class=\"overflow-x-auto\"><table class=\"min-w-full divide-y divide-neutral-200\">';
        html += '<thead class=\"bg-neutral-50\"><tr>';
        html += '<th class=\"px-4 py-2 text-left text-xs font-medium text-neutral-500 uppercase\">Задача</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Всего ошибок</th>';
        html += '<th class=\"px-4 py-2 text-right text-xs font-medium text-neutral-500 uppercase\">Окончательно провалено</th>';
        html += '</tr></thead><tbody class=\"bg-white divide-y divide-neutral-200\">';
      ...
      document.getElementById('recent-errors-container').innerHTML = '<div class=\"text-red-500 text-sm\">Не удалось загрузить данные об ошибках</div>';
    }
  })();
  "
}
