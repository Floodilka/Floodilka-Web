...
pub fn handle_update_flags(
  ...
  case user_result {
    Error(_) -> flash.redirect_with_error(ctx, redirect_url, "Пользователь не найден")
    Ok(option.None) ->
      flash.redirect_with_error(ctx, redirect_url, "Пользователь не найден")
      ...
      case result {
        Ok(_) ->
          flash.redirect_with_success(
            ctx,
            redirect_url,
            "Флаги пользователя обновлены",
          )
        Error(_) ->
          flash.redirect_with_error(
            ctx,
            redirect_url,
            "Не удалось обновить флаги пользователя",
          )
      }
    }
  }
  ...
  case result {
    Ok(_) ->
      flash.redirect_with_success(
        ctx,
        redirect_url,
        "Флаги подозрительной активности обновлены",
      )
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Не удалось обновить флаги подозрительной активности",
      )
  }
}
...
pub fn handle_update_acls(
  ...
  case result {
    ...
    Error(_) ->
      flash.redirect_with_error(ctx, redirect_url, "Не удалось обновить ACL")
  }
}
...
pub fn handle_disable_mfa(
  ...
  case users.disable_mfa(ctx, session, user_id) {
    ...
    Error(_) ->
      flash.redirect_with_error(ctx, redirect_url, "Не удалось отключить MFA")
  }
}
...
pub fn handle_verify_email(
  ...
  case users.verify_email(ctx, session, user_id) {
    ...
    Error(_) ->
      flash.redirect_with_error(ctx, redirect_url, "Не удалось подтвердить Email")
  }
}
...
pub fn handle_unlink_phone(
  ...
  case users.unlink_phone(ctx, session, user_id) {
    ...
    Error(_) ->
      flash.redirect_with_error(ctx, redirect_url, "Не удалось отвязать телефон")
  }
}
      ...
      case users.clear_user_fields(ctx, session, user_id, fields) {
        Ok(_) ->
          flash.redirect_with_success(
            ctx,
            redirect_url,
            "Поля пользователя очищены",
          )
        Error(_) ->
          flash.redirect_with_error(
            ctx,
            redirect_url,
            "Не удалось очистить поля пользователя",
          )
      }
    }
  }
  ...
  case users.set_bot_status(ctx, session, user_id, bot) {
    ...
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Не удалось обновить статус бота",
      )
  }
}
  ...
  case users.set_system_status(ctx, session, user_id, system) {
    Ok(_) ->
      flash.redirect_with_success(
        ctx,
        redirect_url,
        "Системный статус обновлён",
      )
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Не удалось обновить системный статус",
      )
  }
}
...
pub fn handle_change_username(
  ...
  case username {
    "" ->
      flash.redirect_with_error(ctx, redirect_url, "Имя пользователя не может быть пустым")
    _ -> {
      case users.change_username(ctx, session, user_id, username) {
        Ok(_) ->
          flash.redirect_with_success(
            ctx,
            redirect_url,
...
pub fn handle_change_email(
  ...
  case email {
    "" -> flash.redirect_with_error(ctx, redirect_url, "Email не может быть пустым")
    _ -> {
      case users.change_email(ctx, session, user_id, email) {
        ...
        Error(_) ->
          flash.redirect_with_error(ctx, redirect_url, "Не удалось изменить Email")
      }
    }
  }
}
    ...
    users.temp_ban_user(ctx, session, user_id, duration, reason, private_reason)
  {
    ...
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Не удалось временно заблокировать пользователя",
      )
  }
}
...
pub fn handle_unban(
  ...
  case users.unban_user(ctx, session, user_id) {
    ...
    Error(_) ->
      flash.redirect_with_error(ctx, redirect_url, "Не удалось разблокировать пользователя")
  }
}
    ...
    )
  {
    ...
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Не удалось запланировать удаление аккаунта",
      )
  }
}
  ...
  case users.cancel_deletion(ctx, session, user_id) {
    ...
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Не удалось отменить удаление аккаунта",
      )
  }
}
  ...
  case users.cancel_bulk_message_deletion(ctx, session, user_id) {
    Ok(_) ->
      flash.redirect_with_success(
        ctx,
        redirect_url,
        "Массовое удаление сообщений отменено",
      )
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Не удалось отменить массовое удаление сообщений",
      )
  }
}
  ...
  case messages.delete_all_user_messages(ctx, session, user_id, dry_run) {
      ...
      case dry_run {
        True -> {
          ...
          flash.redirect_with_success(
            ctx,
            location,
            "Пробный запуск нашёл "
              <> int.to_string(response.message_count)
              <> " сообщений в "
              <> int.to_string(response.channel_count)
              <> " каналах. Подтвердите для окончательного удаления.",
          )
        }
        False -> {
          ...
          let message = case response.job_id {
            option.Some(_) ->
              "Задание на удаление поставлено в очередь. Отслеживайте прогресс на панели статуса."
            option.None -> "Сообщений для удаления не найдено."
          }
    ...
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        redirect_url,
        "Не удалось удалить все сообщения пользователя",
      )
  }
}
...
pub fn handle_change_dob(
  ...
  case date_of_birth {
    ...
    Error(_) ->
      flash.redirect_with_error(ctx, redirect_url, "Некорректная дата рождения")
  }
}
      ...
      case list.is_empty(entries) {
        True ->
          flash.redirect_with_error(
            ctx,
            redirect_url,
            "CSV не содержит валидных пар channel_id,message_id.",
          )
        False -> {
          let entries_json =
            ...
            messages.queue_message_shred(ctx, session, user_id, entries_json)
          {
            Ok(response) -> {
              ...
              flash.redirect_with_success(
                ctx,
                location,
                "Задание на уничтожение сообщений поставлено в очередь",
              )
            }
            Error(_) ->
              flash.redirect_with_error(
                ctx,
                redirect_url,
                "Не удалось поставить задание на уничтожение сообщений в очередь",
              )
          }
        }
      }
                    ...
                    case int.parse(channel_trim) {
                      Ok(channel_value) ->
                        case int.parse(message_trim) {
                          ...
                          Error(_) ->
                            Error("Некорректный message_id в строке: " <> trimmed)
                        }
                      Error(_) ->
                        Error("Некорректный channel_id в строке: " <> trimmed)
                    }
                  }
                  _ ->
                    Error(
                      "Каждая строка должна содержать channel_id и message_id, разделённые запятой",
                    )
                }
              }
            }
