...
import floodilka_admin/components/review_keyboard
...
(function () {
  ...
  function showToast(message) {
    ...
    toast.innerHTML =
      ...
      '<div class=\"text-sm font-semibold\">Действие не выполнено</div>' +
      '<div class=\"text-sm mt-1\" style=\"word-break: break-word;\">' + (message || 'Неизвестная ошибка') + '</div>' +
      '</div>' +
      '</div>';
  ...
  function enhanceDeck(deck) {
    ...
    function updateProgress() {
      ...
      if (!el) return;
      el.textContent = remainingCount().toString() + ' осталось';
    }
    ...
    function wireButtons(card) {
      ...
      if (leftBtn) {
        leftBtn.addEventListener('click', function (e) {
          e.preventDefault();
          if (window.floodilkaReviewKeyboard) {
            window.floodilkaReviewKeyboard.enable(deck);
          }
          act('left');
        }, { capture: true });
      }
