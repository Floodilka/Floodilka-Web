...
import floodilka_admin/api/voice
...
import floodilka_admin/components/voice as voice_components
...
fn view_no_region(
  ...
  let content =
    h.div([a.class("space-y-6")], [
      h.div([a.class("mb-6")], [ui.heading_page("Голосовые серверы")]),
      h.div(
        [
          a.class(
        ...
        [
          h.p([a.class("text-yellow-800 mb-4")], [
            element.text("Сначала выберите регион."),
          ]),
          h.a(
            [
            ...
            [element.text("Перейти к голосовым регионам")],
          ),
        ],
      ),
    ])
        ...
        ui.flex_row_between([
          h.div([], [
            h.a(
              ...
              [element.text("← Назад к регионам")],
            ),
            ui.heading_page("Серверы: " <> region_name),
          ]),
          h.a(
            [
              a.href("#create"),
...
fn render_servers_list(
  ...
  case list.is_empty(servers) {
    True ->
      ui.card_empty([
        ui.text_muted("Серверы для этого региона ещё не настроены."),
        ui.text_small_muted("Добавьте первый сервер, чтобы начать."),
      ])
    False ->
      h.div(
        [a.class("space-y-4")],
  ...
  h.div(
    ...
    [
      ...
      h.div([a.class("grid grid-cols-2 md:grid-cols-2 gap-4 mb-4")], [
        helpers.info_item("Регион", server.region_id),
        helpers.info_item("Статус", case server.is_active {
          True -> "Активен"
          False -> "Неактивен"
        }),
      ]),
          ...
          [
            h.button(
              [
                ...
                a.attribute(
                  "onclick",
                  "return confirm('Вы уверены, что хотите удалить этот сервер?')",
                ),
              ],
      ...
      h.details([a.class("mt-6")], [
        h.summary(
          ...
          [
            element.text("Редактировать сервер"),
          ],
        ),
      ...
      [
        ...
        h.div([a.class("space-y-1")], [
          h.label([a.class("text-sm font-medium text-neutral-700")], [
            element.text("Ключ API"),
          ]),
          h.input([
            a.type_("text"),
            a.name("api_key"),
            a.placeholder("Оставьте пустым, чтобы не менять"),
            a.class(
          ...
          h.p([a.class("text-xs text-neutral-500")], [
            element.text("Ключ API LiveKit (оставьте пустым, чтобы не менять)"),
          ]),
        ]),
        h.div([a.class("space-y-1")], [
          h.label([a.class("text-sm font-medium text-neutral-700")], [
            element.text("Секрет API"),
          ]),
          h.input([
            a.type_("password"),
            a.name("api_secret"),
          ...
          h.p([a.class("text-xs text-neutral-500")], [
            element.text("Секрет API LiveKit (оставьте пустым, чтобы не менять)"),
          ]),
        ]),
...
fn render_create_form(_ctx: Context, region_id: String) {
  h.div([a.class("bg-white border border-neutral-200 rounded-lg p-6")], [
    h.h2([a.class("text-base font-medium text-neutral-900 mb-4")], [
      element.text("Добавить голосовой сервер"),
    ]),
    h.form(
      [
        a.method("POST"),
