...
import floodilka_admin/api/feature_flags
          ...
          [
            ...
            h.div([a.class("space-y-1")], [
              ...
              h.p([a.class("text-xs text-neutral-500")], [
                element.text(
                  "ID серверов через запятую, которым доступна эта функция.",
                ),
              ]),
            ]),
...
pub fn handle_action(
  ...
  case acl.has_permission(admin_acls, constants.acl_feature_flag_manage) {
    False -> flash.redirect_with_error(ctx, "/feature-flags", "Доступ запрещён")
    True ->
      case action_name {
        Ok("update") -> handle_update(req, ctx, session)
        _ -> flash.redirect_with_error(ctx, "/feature-flags", "Неизвестное действие")
      }
  }
}
...
fn handle_update(req: Request, ctx: Context, session: Session) -> Response {
  ...
  case feature_flags.update_feature_flag(ctx, session, flag_id, guild_ids) {
    Ok(_) ->
      flash.redirect_with_success(ctx, "/feature-flags", "Флаг функции обновлён")
    Error(_) ->
      flash.redirect_with_error(
        ctx,
        "/feature-flags",
        "Не удалось обновить флаг функции",
      )
  }
}
