  ...
  let content = case result {
    Ok(response) ->
      h.div([a.class("space-y-6")], [
        ui.flex_row_between([
          ui.heading_page("Голосовые регионы"),
          h.a(
            [
              a.href("#create"),
              a.class(
...
fn render_regions_list(ctx: Context, regions: List(voice.VoiceRegion)) {
  case list.is_empty(regions) {
    True ->
      ui.card_empty([
        ui.text_muted("Голосовые регионы ещё не настроены."),
        ui.text_small_muted("Создайте первый регион, чтобы начать."),
      ])
    False ->
      h.div(
        [a.class("space-y-4")],
  ...
  h.div(
    ...
    [
        ...
        h.div([a.class("flex items-center gap-3")], [
          ...
          h.div([], [
            ...
            h.p([a.class("text-sm text-neutral-600")], [
              element.text("ID региона: " <> region.id),
            ]),
          ]),
        ]),
      ...
      h.div([a.class("grid grid-cols-2 md:grid-cols-3 gap-4 mb-4")], [
        helpers.info_item("Широта", float.to_string(region.latitude)),
        helpers.info_item("Долгота", float.to_string(region.longitude)),
        helpers.info_item("Серверы", case region.servers {
        ...
        h.form([a.method("POST"), a.action("?action=delete&id=" <> region.id)], [
          h.button(
            [
              ...
              a.attribute(
                "onclick",
                "return confirm('Вы уверены? Это удалит все серверы в этом регионе.')",
              ),
            ],
            [element.text("Удалить регион")],
          ),
        ]),
        h.a(
          [
          ...
          [element.text("Управление серверами")],
        ),
      ]),
      h.details([a.class("mt-6")], [
        h.summary(
          ...
          [
            element.text("Редактировать регион"),
          ],
        ),
        ...
        h.div([a.class("space-y-2")], [
          h.label([a.class("flex items-center gap-2")], [
            ...
            h.span([a.class("text-sm text-neutral-700")], [
              element.text("Сделать регионом по умолчанию"),
            ]),
          ]),
        ]),
...
fn render_create_form(_ctx: Context) {
  h.div([a.class("bg-white border border-neutral-200 rounded-lg p-6")], [
    h.h2([a.class("text-base font-medium text-neutral-900 mb-4")], [
      element.text("Создать голосовой регион"),
    ]),
    h.form(
