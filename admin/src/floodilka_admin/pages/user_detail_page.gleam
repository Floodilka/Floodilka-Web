...
import floodilka_admin/pages/user_detail/handlers
import floodilka_admin/pages/user_detail/tabs/account
import floodilka_admin/pages/user_detail/tabs/guilds
import floodilka_admin/pages/user_detail/tabs/moderation
import floodilka_admin/pages/user_detail/tabs/overview
  ...
  let content = case result {
        ...
        h.div([a.class("mb-6")], [
          h.a(
            ...
            [
              ...
              element.text("Назад к пользователям"),
            ],
          ),
        ]),
        h.div(
              ...
              [
                ...
                h.div([a.class("flex-1")], [
                  h.div([a.class("flex flex-wrap items-center gap-3 mb-3")], [
                    ui.heading_section(user_data.username),
                    case user_data.bot {
                      True ->
                        h.span(
                          [
    ...
    Error(err) ->
      errors.api_error_view(
        ...
        err,
        option.Some("/users"),
        option.Some("Назад к пользователям"),
      )
  }
    ...
    h.div(
      ...
      [
        h.h2([a.class("text-base font-semibold text-neutral-900 mb-2")], [
          element.text("Пользователь не найден"),
        ]),
        h.p([a.class("text-neutral-600 mb-6")], [
          element.text("Запрашиваемый пользователь не найден."),
        ]),
        h.a(
          [
            href(ctx, "/users"),
...
fn archives_tab(ctx: Context, session: Session, user_id: String) {
  ...
  h.div([], [
    ui.flex_row_between([
      ui.heading_section("Архивы пользователя"),
      h.form(
        [
          a.method("post"),
          action(
        ...
        [
          ui.button_primary("Создать архив", "submit", []),
        ],
      ),
    ]),
    case result {
  ...
  case list.is_empty(archives) {
    True ->
      h.div(
        ...
        [
          element.text("Нет архивов для этого пользователя."),
        ],
      )
    False ->
      h.div(
