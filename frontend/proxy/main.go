...
var metricsHost = os.Getenv("FLOODILKA_METRICS_HOST")
...
var staticCDNEndpoint = func() string {
	if v := strings.TrimSpace(os.Getenv("STATIC_CDN_ENDPOINT")); v != "" {
		return v
	}
	return "https://static.floodilka.com"
}()
...
func (s *Server) initAssetsProxy() {
	...
	if err != nil {
		s.errorLog.Printf("Invalid STATIC_CDN_ENDPOINT %q: %v", staticCDNEndpoint, err)
		return
	}
...
func sendMetric(path string, body map[string]any) {
	...
	go func() {
		...
		req.Header.Set("User-Agent", "floodilka-proxy/metrics")
...
func (s *Server) handleAppleAppSiteAssociation(w http.ResponseWriter) {
  ...
  "webcredentials": {
    "apps": [
      "4K3W3NBQTP.com.floodilka.desktop",
      "4K3W3NBQTP.com.floodilka.desktop.canary"
    ]
  }
}`
