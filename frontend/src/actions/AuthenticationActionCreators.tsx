...
const withPlatformHeader = (headers?: Record<string, string>): Record<string, string> => ({
	'X-Floodilka-Platform': getPlatformHeaderValue(),
	...(headers ?? {}),
});
...
export const register = async (data: RegisterData): Promise<{ticket: string}> => {
	try {
		const headers: Record<string, string> = {};
		...
		const response = await http.post<{ticket: string}>({
			url: Endpoints.AUTH_REGISTER,
			body: bodyData,
			headers: withPlatformHeader(headers),
		});
		logger.info('Registration successful, ticket received');
		return response.body;
	} catch (error) {
...
export const verifyRegistrationCode = async (ticket: string, code: string): Promise<TokenResponse> => {
	try {
		const response = await http.post<TokenResponse>({
			url: Endpoints.AUTH_REGISTER_VERIFY,
			body: {ticket, code},
			headers: withPlatformHeader(),
		});
		logger.info('Registration code verified');
		return response.body;
	} catch (error) {
		logger.error('Registration code verification failed', error);
		throw error;
	}
};

export const resendRegistrationCode = async (ticket: string): Promise<void> => {
	await http.post({
		url: Endpoints.AUTH_REGISTER_RESEND,
		body: {ticket},
		headers: withPlatformHeader(),
	});
	logger.info('Registration code resent');
};
...
export const verifyResetCode = async (email: string, code: string): Promise<{resetToken: string}> => {
	try {
		...
		return {resetToken: response.body.resetToken};
	} catch (error) {
