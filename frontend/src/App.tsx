...
export const AppWrapper = observer(({children}: AppWrapperProps) => {
	...
	React.useEffect(() => {
		const htmlNode = document.documentElement;
		htmlNode.classList.remove('theme-light', 'theme-coal', 'theme-system');
	...
	return (
		<LayoutVariantProvider value={layoutVariantContextValue}>
			...
			<RoomContext.Provider value={room ?? undefined}>
				...
				{isNative && !isMacOS && layoutVariant !== 'app' && (
					<div className={styles.standaloneWindowControls}>
						<NativeTitlebar platform={platform} />
					</div>
				)}
				<div ref={ringsContainerRef} className={styles.appContainer}>
					<FocusRingScope containerRef={ringsContainerRef}>
						{layoutVariant === 'app' && <header className={styles.workspaceHeader}>
							<div className={styles.workspaceHeaderInner}>
								<span className={styles.workspaceHeaderTitle}>Флудилка</span>
