...
const DEFAULT_GATEWAY_HOSTS = new Set(['gateway.floodilka.com', 'gateway.stage.floodilka.com']);
...
class RuntimeConfigStore {
	...
	private async initialize(): Promise<void> {
		try {
			...
			const cachedEndpoint = this.apiEndpoint;
			const defaultEndpoint = Config.PUBLIC_BOOTSTRAP_API_ENDPOINT;
			const bootstrapEndpoint = cachedEndpoint || defaultEndpoint;

			try {
				await this.connectToEndpoint(bootstrapEndpoint);
			} catch (e) {
				if (cachedEndpoint && cachedEndpoint !== defaultEndpoint) {
					runInAction(() => {
						this.apiEndpoint = '';
					});
					await this.connectToEndpoint(defaultEndpoint);
				} else {
					throw e;
				}
			}
	...
	isThirdPartyGateway(): boolean {
		...
		try {
			...
			return !DEFAULT_GATEWAY_HOSTS.has(url.hostname);
		} catch {
			return false;
		}
	}
