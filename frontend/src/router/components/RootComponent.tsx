...
const RootComponent: React.FC<{children?: React.ReactNode}> = observer(({children}) => {
	...
	const shouldBypassGateway = isAuthRoute;
	...
	React.useEffect(() => {
		...
		if (isPublicPage && isDesktop()) {
			RouterUtils.replaceWith(isAuth ? Routes.ME : Routes.LOGIN);
			return;
		}
	...
	if (isDesktop() && !SessionManager.isInitialized) {
		return <SplashScreen />;
	}

	if (isAuthenticated && !canNavigateToProtectedRoutes && !shouldBypassGateway && !isPublicPage) {
		return <SplashScreen />;
	}
