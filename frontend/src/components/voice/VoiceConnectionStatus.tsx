...
const VoiceConnectionStatusInner = observer(() => {
	...
	const connectionId = MediaEngineStore.connectionId;
	const isMobile = voiceState?.is_mobile ?? false;
	const noiseSuppression = VoiceSettingsStore.noiseSuppression;
		...
		<div className={styles.voiceConnectionContainer}>
			<div className={styles.statusRow}>
				...
				<div className={styles.controls}>
					<Tooltip text={noiseSuppression ? t`Disable Noise Suppression` : t`Enable Noise Suppression`}>
						<FocusRing offset={-2}>
							<button
								type="button"
								className={clsx(styles.controlButton, noiseSuppression && styles.selected)}
								onClick={() => VoiceSettingsActionCreators.update({noiseSuppression: !noiseSuppression})}
								aria-label={noiseSuppression ? t`Disable Noise Suppression` : t`Enable Noise Suppression`}
							>
