...
export const IncomingCallManager: React.FC = observer(function IncomingCallManager() {
	...
	const {nextPopoutIds, popoutModels, hasRingingCalls} = useMemo(() => {
		...
		for (const call of calls) {
			...
			const channel = ChannelStore.getChannel(call.channelId);
			let initiatorUserId: string | undefined;
			if (channel?.type === ChannelTypes.DM) {
				initiatorUserId = channel.recipientIds.find((id) => id !== currentUserId);
			} else {
				initiatorUserId = call.participants[0] ?? call.ringing[0];
			}
