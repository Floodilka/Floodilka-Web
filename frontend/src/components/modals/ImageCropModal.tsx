	...
	({
		...
		const handleSave = React.useCallback(async () => {
			...
			try {
				...
				if (isGif) {
					try {
						outBlob = await exportAnimatedGif(
							img,
						...
						console.warn('GIF crop failed, falling back to original:', gifError);
						if (onSkip) {
							ToastActionCreators.createToast({
								type: 'info',
								children: t`Could not crop this GIF. The original image will be used instead.`,
							});
							onSkip();
					...
					outBlob = await exportStaticImage(
						img,
						scaledDisplayDimensions,
						cropDimensions,
						transformRef.current,
