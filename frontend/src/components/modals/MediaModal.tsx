...
const FileInfo: FC<FileInfoProps> = observer(
	({fileName, fileSize, dimensions, currentIndex, totalAttachments, onPrevious, onNext}) => {
		const {t} = useLingui();
...
const Controls: FC<ControlsProps> = observer(
	({
		...
		const hasActions = onFavorite || onSave || onInfo || additionalActions;
							...
							<div className={styles.headerBar}>
								{zoomState !== 'zoomed' && authorName && (
									<div className={styles.headerAuthor}>
										...
										<div className={styles.headerAuthorInfo}>
											<span className={styles.headerAuthorName}>{authorName}</span>
											{timestamp && (
												<span className={styles.headerAuthorTime}>
													{timestamp.toLocaleDateString(undefined, {day: 'numeric', month: 'long'})}
													{' '}
