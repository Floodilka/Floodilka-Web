...
export const VoiceTab: React.FC<VoiceTabProps> = observer(
	({voiceSettings, hasPremium, autoRequestPermission = true}) => {
		...
		const inputDevices = React.useMemo(() => devices.filter((d) => d.kind === 'audioinput'), [devices]);
		const outputDevices = React.useMemo(() => devices.filter((d) => d.kind === 'audiooutput'), [devices]);
					...
					modal(() => (
						<ConfirmModal
							...
							description={
								<p>
									<Trans>
										In your browser, Push-to-Talk will only work when the Флудилка tab is focused. For system-wide
							...
							onPrimary={() => {
								void openExternalUrl(`${window.location.origin}/download`);
							}}
							onSecondary={() => {
		...
		return (
			<>
						...
						<div className={styles.deviceNoticeText}>
							...
							<p className={styles.deviceNoticeDescription}>
								{permissionStatus === 'denied' ? (
									isNativeDesktop ? (
										<Trans>
											Allow Флудилка to access your microphone in System Settings → Privacy &amp; Security → Microphone.
										</Trans>
									) : (
										<Trans>
											Allow Флудилка to access your microphone. Check your browser address bar or settings to enable
											permissions.
										</Trans>
									)
								) : (
									<Trans>Флудилка needs access to list the available microphones and speakers.</Trans>
								)}
							</p>
						</div>
						<Button
								...
								link={
									!isNativeDesktop
										? {
												...
												onClick: () => void openExternalUrl(`${window.location.origin}/download`),
											}
										: {
				...
				<Switch
					...
					description={<Trans>Floodilka AI filters keyboard sounds, mouse clicks, and background noise</Trans>}
				...
				<EntranceSoundSection hasPremium={hasPremium} />
			</>
		);
	},
);
