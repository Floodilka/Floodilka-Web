			...
			try {
				...
				if (file.type === 'image/gif' && !hasPremium) {
					ModalActionCreators.push(
						modal(() => (
							<ConfirmModal
								title={t`Animated Avatars Require Premium`}
								description={
									<>
										<p>
											<Trans>
												Animated avatars (GIFs) are a premium feature exclusively available to Premium subscribers.
											</Trans>
										</p>
										<p className={styles.spacedParagraph}>
											<Trans>
												With Premium, you can use animated GIFs for both your avatar and banner, write longer bios
												(up to {MAX_BIO_LENGTH_PREMIUM} characters), and unlock many other premium features.
											</Trans>
										</p>
									</>
								}
				...
				{!isPerGuildProfile && (
					<div className={styles.description}>
						{hasPremium ? (
							...
							<Trans>
								JPEG, PNG, WebP. Max 10MB. Recommended: 512×512px. Animated avatars (GIF) require <PremiumLink />.
							</Trans>
						)}
					</div>
				)}
