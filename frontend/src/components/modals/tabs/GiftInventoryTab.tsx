	...
	const handleRedeem = async () => {
		...
		try {
			...
			ToastActionCreators.createToast({
				type: 'success',
				children: <Trans>Gift redeemed successfully! Enjoy your Premium!</Trans>,
			});
			onRedeemSuccess();
		} catch (error) {
						...
						{isRedeemed ? (
							<div className={styles.redeemedMessage}>
								<p className={styles.redeemedMessageText}>
									{gift.redeemed_by ? (
										<Trans>
											Redeemed by {gift.redeemed_by.username}
										</Trans>
									) : (
	...
	if (isUnclaimed) {
		return (
			<StatusSlate
				...
				description={<Trans>Claim your account to redeem or manage Premium gift codes.</Trans>}
				actions={[
					{
		...
		<div className={styles.container}>
			<div>
				...
				<p className={styles.sectionDescription}>
					<Trans>Enter a gift code to redeem Premium for your account.</Trans>
				</p>
			...
			<div>
				...
				<p className={styles.sectionDescriptionNoMargin}>
					<Trans>
						Manage your purchased Premium gift codes. Share the gift URL with someone special or redeem it for
						yourself!
					</Trans>
				</p>
			</div>
			...
			{!loading && !error && gifts.length === 0 && (
				<StatusSlate
					...
					description={<Trans>Purchase a Premium gift from the Premium tab to share with friends!</Trans>}
					actions={[
						{
							text: <Trans>Go to Premium</Trans>,
							onClick: () => ComponentDispatch.dispatch('USER_SETTINGS_TAB_SELECT', {tab: 'premium'}),
							variant: 'primary',
							fitContent: true,
						},
					]}
