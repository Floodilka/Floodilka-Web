...
const ProfileModalContent: React.FC<ProfileModalContentProps> = observer(
	({profile, user, userNote, autoFocusNote, noteRef, renderActionButtons, previewOverrides, warningIndicator}) => {
		const {t} = useLingui();
		const effectiveProfile = profile?.getEffectiveProfile() ?? null;
		const bannerColor = (effectiveProfile?.banner_color != null ? ColorUtils.int2hex(effectiveProfile.banner_color) : null) || DEFAULT_ACCENT_COLOR;
				...
				<header>
						...
						<svg className={userProfileModalStyles.bannerMask} viewBox="0 0 600 210" preserveAspectRatio="none">
							...
							<foreignObject x="0" y="0" width="600" height="210" overflow="visible" mask={`url(#${maskId})`}>
								{bannerUrl ? (
									...
									<div className={userProfileModalStyles.bannerColor} style={{background: 'linear-gradient(135deg, #5865f2 0%, #9b59b6 50%, #8e44ad 100%)'}} />
								)}
							</foreignObject>
						</svg>
					</div>
					...
					<div className={userProfileModalStyles.headerContainer}>
						<div className={userProfileModalStyles.avatarContainer}>
							<StatusAwareAvatar size={120} user={user} avatarUrl={avatarUrl} hoverAvatarUrl={hoverAvatarUrl} forceAnimate={true} />
						</div>
...
export const UserProfileModal: UserProfileModalComponent = observer(
	({userId, guildId, autoFocusNote, disableEditProfile, previewOverrides, previewUser}) => {
		...
		const handleCopyUsername = () => {
			TextCopyActionCreators.copy(i18n, displayUser.username, true);
		};
		...
		const borderColor = (borderProfile?.banner_color != null ? ColorUtils.int2hex(borderProfile.banner_color) : null) || DEFAULT_ACCENT_COLOR;
		...
		return (
			...
			>
				...
				<div className={userProfileModalStyles.modalContainer}>
					{shouldShowSpinner ? (
