...
export const ProfilePreview: React.FC<ProfilePreviewProps> = observer(
	({
		...
		const previewUser = React.useMemo(() => {
			...
			return user.withUpdates({bio, global_name: globalName});
		}, [user, previewBio, previewGlobalName]);
		...
		const bannerColor = (user.bannerColor != null ? ColorUtils.int2hex(user.bannerColor) : null) || DEFAULT_ACCENT_COLOR;
