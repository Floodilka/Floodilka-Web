...
export const ChannelItem = observer(
	({
		...
		const canConnect = !channelIsVoice || PermissionStore.can(Permissions.CONNECT, channel);
		...
		const handleSelect = useCallback(() => {
			...
			if (channel.type === ChannelTypes.GUILD_VOICE && !canConnect) {
				return;
			}
		...
		const channelItem = (
			<GenericChannelItem
				...
				className={clsx(
					...
					(voiceBlockedForUnclaimed || (channelIsVoice && !canConnect)) && styles.channelItemDisabled,
				)}
