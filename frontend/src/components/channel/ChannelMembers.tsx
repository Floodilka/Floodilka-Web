...
export const ChannelMembers = observer(({guild = null, channel}: ChannelMembersProps) => {
	...
	const memberCount = MemberSidebarStore.getMemberCount(guild.id, channel.id);
	const frameSides = guild ? {left: false} : undefined;

	return (
		<div className={styles.memberListOuter}>
			{memberCount > 0 && (
				<div className={styles.memberCountHeader}>
					{t`Members`} — {memberCount}
				</div>
			)}
			<OutlineFrame hideTopBorder sides={frameSides} className={styles.memberListFrame}>
