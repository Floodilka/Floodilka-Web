...
export const GifPickerGridItem = React.memo(function GifPickerGridItem({
	...
	const handleClick = (event: React.MouseEvent<HTMLDivElement> | React.KeyboardEvent<HTMLDivElement>) => {
		...
		KlipyActionCreators.registerShare(gif.id, searchTerm);
	...
	if (item.type === 'category') {
		...
		const isImagePreview = item.previewProxySrc?.endsWith('.gif') || item.previewUrl?.endsWith('.gif');
		...
		return (
			...
			>
				<div className={styles.gifMediaContainer}>
					{isImagePreview ? (
						<img
							src={item.previewProxySrc || item.previewUrl}
							alt={item.title}
	...
	const isFavorited = FavoriteMemeUtils.isFavoritedByKlipyId(favoriteMemes, gif.id);
	...
	const handleFavoriteClick = async (e: React.MouseEvent) => {
		...
		try {
			if (isFavorited) {
				const meme = FavoriteMemeUtils.findFavoritedMeme(favoriteMemes, {klipyId: gif.id});
				if (meme) {
