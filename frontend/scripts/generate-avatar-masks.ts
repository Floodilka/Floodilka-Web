...
const STATUS_CONFIG: Record<number, StatusConfig> = {
	16: {statusSize: 10, cutoutRadius: 5, cutoutCenter: 15},
	20: {statusSize: 10, cutoutRadius: 5, cutoutCenter: 19},
	24: {statusSize: 10, cutoutRadius: 7, cutoutCenter: 22},
	32: {statusSize: 10, cutoutRadius: 8, cutoutCenter: 30},
	36: {statusSize: 10, cutoutRadius: 8, cutoutCenter: 34},
	40: {statusSize: 12, cutoutRadius: 9, cutoutCenter: 38},
	48: {statusSize: 14, cutoutRadius: 10, cutoutCenter: 45},
	56: {statusSize: 16, cutoutRadius: 11, cutoutCenter: 53},
	80: {statusSize: 16, cutoutRadius: 14, cutoutCenter: 75},
	120: {statusSize: 24, cutoutRadius: 20, cutoutCenter: 113},
};
...
const AVATAR_CORNER_RADIUS_RATIO = 0.2;

function generateAvatarMaskDefault(size: number): string {
	const rx = Math.round(size * AVATAR_CORNER_RADIUS_RATIO);
	return `<rect fill="white" x="0" y="0" width="${size}" height="${size}" rx="${rx}" ry="${rx}" />`;
}
...
function generateAvatarMaskStatusRound(size: number): string {
	...
	return `(
				<>
					<rect fill="white" x="0" y="0" width="${size}" height="${size}" rx="${rx}" ry="${rx}" />
...
for (const size of SIZES) {
	...
	const normalizedRx = AVATAR_CORNER_RADIUS_RATIO;

	output += `			<mask id="svg-mask-avatar-default-${size}" maskContentUnits="objectBoundingBox" viewBox="0 0 1 1">
				<rect fill="white" x="0" y="0" width="1" height="1" rx="${normalizedRx}" ry="${normalizedRx}" />
			</mask>
...
output += `			<mask id="svg-mask-status-online" maskContentUnits="objectBoundingBox" viewBox="0 0 1 1">
			...
			<mask id="svg-mask-avatar-default" maskContentUnits="objectBoundingBox" viewBox="0 0 1 1">
				<rect fill="white" x="0" y="0" width="1" height="1" rx="${AVATAR_CORNER_RADIUS_RATIO}" ry="${AVATAR_CORNER_RADIUS_RATIO}" />
			</mask>
		</defs>
	</svg>
);
