...
export class AvatarService {
	...
	async uploadAvatar(params: {
		...
		const isAnimated = metadata.animated ?? (metadata.format === 'gif');
		return isAnimated ? `a_${imageHashShort}` : imageHashShort;
	}
	...
	async processEmoji(params: {errorPath: string; base64Image: string}): Promise<{
		...
		return {imageBuffer, animated: metadata.animated ?? (metadata.format === 'gif')};
	}
