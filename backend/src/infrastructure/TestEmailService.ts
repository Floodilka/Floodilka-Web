...
export class TestEmailService implements ITestEmailService {
	...
	async sendPasswordResetCode(
		...
		Logger.info(`[TestEmailService] Password reset code sent to ${email} for user ${username}`);
		Logger.info(`[TestEmailService] Reset code: ${code}`);
		return this.recordEmail({
			to: email,
	...
	async sendRegistrationCode(
		...
		Logger.info(`[TestEmailService] Registration code sent to ${email} for user ${username}`);
