...
export class EntityAssetService {
	...
	private buildCdnUrlBase(assetType: AssetType, entityType: EntityType, entityId: bigint, guildId?: bigint): string {
		const prefix = ASSET_TYPE_TO_URL_PREFIX[assetType];
	...
	private async validateAndProcessImage(
		base64Image: string,
		errorPath: string,
	): Promise<{imageBuffer: Uint8Array; format: string; animated: boolean; height?: number; width?: number}> {
		...
		return {
			...
			animated: metadata.animated ?? (metadata.format === 'gif'),
