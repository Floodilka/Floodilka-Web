...
export class AuthRegistrationService {
	...
	async register({
		...
		if (!usernameCandidate) {
			...
			if (derivedUsername) {
				const existing = await this.repository.findByUsername(derivedUsername);
				if (!existing) {
		...
		if (rawEmail) {
			this.emailService.sendRegistrationCode(rawEmail, username, code, userLocale).catch((error) => {
				Logger.warn({error, email: rawEmail}, 'Failed to send registration code email');
			});
		}
	...
	async verifyRegistrationCode({
		...
		const redisKey = `${PENDING_REG_PREFIX}${ticket}`;
		...
		return this.completeRegistration({pending, request, requestCache});
	}

	async resendRegistrationCode({ticket}: {ticket: string}): Promise<void> {
		const redisKey = `${PENDING_REG_PREFIX}${ticket}`;
		const pending = await this.cacheService.get<PendingRegistration>(redisKey);
		...
		const rawEmail = pending.data.email ?? null;
		if (rawEmail) {
			this.emailService.sendRegistrationCode(rawEmail, pending.username, newCode, pending.locale).catch((error) => {
				Logger.warn({error, email: rawEmail}, 'Failed to send registration code email (resend)');
			});
		}
	}
	...
	private async sendRegistrationWebhook(
		...
		const embedFields = [
			...
			{name: 'Username', value: context.username, inline: true},
