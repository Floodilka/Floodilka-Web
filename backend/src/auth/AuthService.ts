...
export class AuthService implements IAuthService {
	...
	constructor(
		...
		emailDnsValidationService: EmailDnsValidationService,
	...
	async verifyRegistrationCode({
		...
		return this.registrationService.verifyRegistrationCode({ticket, code, request, requestCache});
	}

	async resendRegistrationCode({ticket}: {ticket: string}): Promise<void> {
		return this.registrationService.resendRegistrationCode({ticket});
	}
	...
	async verifyResetCode({data}: VerifyResetCodeParams): Promise<{resetToken: string}> {
		return this.passwordService.verifyResetCode({data});
	}
