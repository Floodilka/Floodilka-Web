...
import {InviteTypes, UserFlags} from '~/Constants';
import type {IChannelRepository} from '~/channel/IChannelRepository';
import * as MessageHelpers from '~/channel/services/message/MessageHelpers';
import type {DSAReportTicketRow, MessageAttachment} from '~/database/types/ReportTypes';
import {
	CannotReportOwnGuildError,
	CannotReportOwnMessageError,
	CannotReportYourselfError,
	InvalidDsaReportTargetError,
	InvalidDsaTicketError,
	InvalidDsaVerificationCodeError,
	RateLimitError,
	ReportBannedError,
	UnknownChannelError,
	UnknownGuildError,
...
import type {DsaReportRequest} from './ReportModel';
...
const DSA_VERIFICATION_CODE_TTL_MS = 10 * 60 * 1000;
const DSA_TICKET_TTL_MS = 60 * 60 * 1000;
...
export class ReportService {
	...
	constructor(
		...
		private reportSearchService: ReportSearchService | null = null,
	) {
		this.cleanupIntervalId = setInterval(() => this.cleanupRateLimitMap(), 5 * 60 * 1000);
	}
	...
	async reportMessage(
		...
		const reportId = createReportID(this.snowflakeService.generate());
		...
		if (this.reportSearchService) {
			await this.reportSearchService.indexReport(report).catch((error) => {
				console.error('Failed to index report in search:', error);
			});
		}
	...
	async sendDsaReportVerificationCode(email: string): Promise<void> {
		...
		const expiresAt = new Date(Date.now() + DSA_VERIFICATION_CODE_TTL_MS);
		...
		this.emailService.sendDsaReportVerificationCode(normalizedEmail, verificationCode, expiresAt).catch((error) => {
			Logger.warn({error, email: normalizedEmail}, 'Failed to send DSA report verification code');
		});
	}
	...
	async verifyDsaReportEmail(email: string, code: string): Promise<string> {
		...
		await this.reportRepository.createDsaTicket({
			ticket,
			email_lower: normalizedEmail,
			expires_at: new Date(Date.now() + DSA_TICKET_TTL_MS),
			created_at: new Date(),
		});
	...
	async createDsaReport(report: DsaReportRequest): Promise<IARSubmission> {
		...
		if (this.reportSearchService) {
			await this.reportSearchService.indexReport(createdReport).catch((error) => {
				console.error('Failed to index DSA report in search:', error);
			});
		}
	...
	private async buildDsaUserReportRow(
		...
		const target = await this.resolveDsaUser(report.user_id);
	...
	private async resolveDsaUser(userId?: bigint): Promise<User> {
		if (userId != null) {
	...
	async resolveReport(
		...
		if (this.reportSearchService) {
			await this.reportSearchService.updateReport(report).catch((error) => {
				console.error('Failed to update report in search:', error);
			});
		}
	...
	private async gatherMessageContext(
		...
		for (const message of allMessages) {
			const author = message.authorId ? users.get(message.authorId) : null;
			if (!author) continue;
			...
			context.push({
				...
				timestamp: new Date(SnowflakeUtils.extractTimestamp(message.id)),
	...
	private async cloneAttachmentsForReport(
		...
		for (const attachment of attachments) {
			...
			try {
				...
				console.error(`Failed to clone attachment ${attachment.id}:`, error);
			}
		}
