...
import {KlipyService as ProdKlipyService} from '~/klipy/KlipyService';
...
const KlipyService = ProdKlipyService;
...
export function shutdownGatewayService(): void {
...
export const ServiceMiddleware = createMiddleware<HonoEnv>(async (ctx, next) => {
	...
	const gatewayService = gatewayServiceInstance;
	...
	const emailDnsValidationService = new EmailDnsValidationService();
	...
	if (paymentService) {
		ctx.set('paymentService', paymentService);
	}
