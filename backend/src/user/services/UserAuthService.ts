...
export class UserAuthService {
	...
	async disableMfaTotp(params: {user: User; code: string; sudoContext: SudoVerificationResult}): Promise<void> {
		...
		// Skip TOTP code verification if identity was already verified via MFA during sudo mode,
		// because the same code was already consumed by the anti-reuse cache in verifySudoMfa.
		if (sudoContext.method !== 'mfa') {
			if (
	...
	async resendVerificationEmail(user: User): Promise<boolean> {
		...
		this.emailService.sendEmailVerification(user.email!, user.username, verificationToken, user.locale).catch((error) => {
