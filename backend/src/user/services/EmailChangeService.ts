...
export class EmailChangeService {
	...
	constructor(
		...
		private readonly emailDnsValidationService: EmailDnsValidationService,
	...
	async start(user: User): Promise<StartEmailChangeResult> {
		...
		if (requireOriginal) {
			...
			this.emailService.sendEmailChangeOriginal(user.email!, user.username, originalCode, user.locale).catch((error) => {
				Logger.warn({error, userId: user.id.toString()}, 'Failed to send email change original code');
			});
		}
	...
	async resendOriginal(user: User, ticket: string): Promise<void> {
		...
		this.emailService.sendEmailChangeOriginal(row.original_email, user.username, originalCode, user.locale).catch((error) => {
			Logger.warn({error, userId: user.id.toString()}, 'Failed to send email change original code (resend)');
		});
	...
	async requestNewEmail(
		...
		this.emailService.sendEmailChangeNew(trimmedEmail, user.username, newCode, user.locale).catch((error) => {
			Logger.warn({error, userId: user.id.toString()}, 'Failed to send email change new code');
		});
	...
	async resendNew(user: User, ticket: string): Promise<void> {
		...
		this.emailService.sendEmailChangeNew(row.new_email, user.username, newCode, user.locale).catch((error) => {
			Logger.warn({error, userId: user.id.toString()}, 'Failed to send email change new code (resend)');
		});
