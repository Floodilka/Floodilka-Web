	...
	try {
		...
		const userData = {
			user: {
				...
				cloudpayments_subscription_id: user.cloudpaymentsSubscriptionId,
				cloudpayments_token: user.cloudpaymentsToken ? '***REDACTED***' : null,
			...
			gift_codes_created: createdGiftCodes.map((gift) => ({
				...
				cloudpayments_transaction_id: gift.cloudpaymentsTransactionId,
			})),
			payments: payments.map((payment) => ({
				...
				cloudpayments_transaction_id: payment.cloudpaymentsTransactionId,
				cloudpayments_subscription_id: payment.cloudpaymentsSubscriptionId,
		...
		const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'floodilka-harvest-'));
		...
		try {
			...
			if (progressReporter.shouldSendEmail && user.email && Config.email.enabled) {
				emailService.sendHarvestCompletedEmail(
					user.email,
					user.username,
					downloadUrl,
					totalMessages,
					...
					Logger.warn({error, userId, harvestId}, 'Failed to send harvest completion email');
				});
