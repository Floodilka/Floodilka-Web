...
export class FavoriteMemeService {
	...
	async createFromUrl({
		...
		if (klipyId) {
			try {
				const memeUrl = `https://klipy.com/gif/${klipyId}`;
				const unfurled = await this.unfurlerService.unfurl(memeUrl, true);
				if (unfurled.length > 0 && unfurled[0].video?.content_hash) {
					contentHash = unfurled[0].video.content_hash;
					Logger.debug({klipyId, contentHash}, 'Using unfurled video content_hash for GIF');
				}
			} catch (error) {
				Logger.warn({error, klipyId}, 'Failed to unfurl GIF URL, using original content_hash');
			}
		}
