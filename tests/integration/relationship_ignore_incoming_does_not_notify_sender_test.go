...
func TestRelationshipIgnoreIncomingDoesNotNotifySender(t *testing.T) {
	...
	resp, err = client.delete(fmt.Sprintf("/users/@me/relationships/%s", alice.UserID), bob.Token)
	if err != nil {
		t.Fatalf("failed to ignore incoming friend request: %v", err)
	}
	...
	if len(bobRels) != 0 {
		t.Fatalf("expected bob to have no relationships after ignoring request, got %d", len(bobRels))
	}
}
