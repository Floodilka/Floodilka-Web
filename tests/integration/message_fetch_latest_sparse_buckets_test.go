...
func TestMessageFetchLatestSparseBucketsBeyondIndexLimit(t *testing.T) {
	client := newTestClient(t)
	account := createTestAccount(t, client)

	guild := createGuild(t, client, account.Token, "Latest Sparse Buckets Test")
	...
	if len(seed.Messages) != bucketCount {
		t.Fatalf("expected %d messages, got %d", bucketCount, len(seed.Messages))
	}

	const limit = 50
	resp, err := client.getWithAuth(
		fmt.Sprintf("/channels/%d/messages?limit=%d", channelID, limit),
		account.Token,
	)
	if err != nil {
	...
	if messages[0].ID != seed.Messages[bucketCount-1].MessageID {
		t.Fatalf("latest message mismatch")
	}
}
