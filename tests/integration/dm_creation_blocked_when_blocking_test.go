...
// TestDMCreationBlockedWhenBlocking verifies that a user cannot create a DM with
// someone they have blocked, even if they share a guild.
func TestDMCreationBlockedWhenBlocking(t *testing.T) {
	...
	if resp.StatusCode != http.StatusForbidden && resp.StatusCode != http.StatusBadRequest {
		t.Fatalf("expected 403 or 400 for DM to blocked user, got %d", resp.StatusCode)
	}
}
