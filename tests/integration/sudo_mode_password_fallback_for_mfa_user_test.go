...
// TestSudoModePasswordRejectedForMfaUser verifies that MFA-enabled users
// cannot use their password for sudo verification - they MUST use an MFA
// method (TOTP, SMS, or WebAuthn).
func TestSudoModePasswordRejectedForMfaUser(t *testing.T) {
	...
	if resp.StatusCode != http.StatusForbidden {
		t.Fatalf("expected 403 Forbidden when MFA user tries password for sudo, got %d: %s", resp.StatusCode, readResponseBody(resp))
	}
	...
	t.Logf("correctly rejected password verification for MFA user")
}
