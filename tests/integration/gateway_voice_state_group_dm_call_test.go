...
func TestGatewayVoiceStateGroupDMCall(t *testing.T) {
	...
	t.Run("join group DM voice call", func(t *testing.T) {
		gatewayClient.SendVoiceStateUpdate(nil, &groupDM.ID, nil, false, false, false, false)
		...
		if vsu.GuildID != nil {
			t.Fatalf("expected guild_id to be null for group DM call, got %v", vsu.GuildID)
		}
		if vsu.ConnectionID == "" {
		...
		t.Logf("Received group DM voice server update: endpoint=%s, connection_id=%s",
		...
		t.Logf("Successfully connected to LiveKit group DM room with %d participant(s)", lkConn.getParticipantCount())
		...
		if vs.ChannelID == nil || *vs.ChannelID != groupDM.ID {
			t.Fatalf("expected channel_id %s, got %v", groupDM.ID, vs.ChannelID)
		}
		if vs.GuildID != nil {
			t.Fatalf("expected guild_id to be null for group DM call, got %v", vs.GuildID)
		}
	})
}
