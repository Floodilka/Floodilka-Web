...
func TestGuildChannelSlowmodeUpdate(t *testing.T) {
	...
	guild := createGuild(t, client, owner.Token, "Slowmode Test Guild")
	...
	if err != nil {
		t.Fatalf("failed to update channel slowmode: %v", err)
	}
	assertStatus(t, resp, http.StatusOK)
	...
	if updatedChannel.RateLimitPerUser != 10 {
		t.Fatalf("expected rate_limit_per_user to be 10, got %d", updatedChannel.RateLimitPerUser)
	}
