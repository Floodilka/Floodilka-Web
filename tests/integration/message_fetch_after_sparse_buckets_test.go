...
func TestMessageFetchAfterSparseBucketsBeyondIndexLimit(t *testing.T) {
	client := newTestClient(t)
	account := createTestAccount(t, client)

	guild := createGuild(t, client, account.Token, "After Sparse Buckets Test")
	...
	anchor := seed.Messages[0].MessageID // oldest
	const limit = 50

	resp, err := client.getWithAuth(
		fmt.Sprintf("/channels/%d/messages?after=%s&limit=%d", channelID, anchor, limit),
		account.Token,
	)
	if err != nil {
