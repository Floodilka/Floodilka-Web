...
// TestEmbedAttachmentURL_BothImageAndThumbnail tests single embed using attachment:// for both fields
func TestEmbedAttachmentURL_BothImageAndThumbnail(t *testing.T) {
	client := newTestClient(t)
	user := createTestAccount(t, client)
	ensureSessionStarted(t, client, user.Token)

	guild := createGuild(t, client, user.Token, "Both Fields Test Guild")
	...
	// Create multipart form with embed using both image and thumbnail
	...
	if strings.Contains(embed.Thumbnail.URL, "attachment://") {
		t.Errorf("expected thumbnail attachment:// URL to be resolved, got '%s'", embed.Thumbnail.URL)
	}
}
