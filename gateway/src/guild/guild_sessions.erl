    ...
    State1 =
        case DisconnectingSession of
            ...
            Session ->
                ...
                cleanup_voice_states_for_session(SessionId, StateAfterSubs)
        end,
...
cleanup_voice_states_for_session(SessionId, State) ->
    VoiceStates = voice_state_utils:voice_states(State),
    SessionVoiceStates = voice_state_utils:filter_voice_states(VoiceStates, fun(_ConnId, VS) ->
        maps:get(<<"session_id">>, VS, undefined) =:= SessionId
    end),
    PendingConnections = maps:get(pending_voice_connections, State, #{}),
    NewPendingConnections = maps:filter(fun(_ConnId, Meta) ->
        maps:get(session_id, Meta, undefined) =/= SessionId
    end, PendingConnections),
    State1 = maps:put(pending_voice_connections, NewPendingConnections, State),
    case maps:size(SessionVoiceStates) of
        0 ->
            State1;
        _ ->
            maps:fold(
                fun(ConnId, VS, AccState) ->
                    UserId = voice_state_utils:voice_state_user_id(VS),
                    case UserId of
                        undefined ->
                            AccState;
                        _ ->
                            Request = #{user_id => UserId, connection_id => ConnId},
                            case guild_voice_disconnect:disconnect_voice_user(Request, AccState) of
