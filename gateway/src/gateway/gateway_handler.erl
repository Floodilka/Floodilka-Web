...
-record(state, {
    ...
    socket_pid,
    peer_ip,
    rate_limit_state = #{events => [], request_guild_members_events => [], window_start => undefined},
        ...
        true ->
            case gateway_codec:decode(Data, Encoding) of
                {ok, #{<<"op">> := Op} = Payload} ->
                    ...
                    case check_rate_limit(NewState, OpAtom) of
                        {ok, RateLimitedState} ->
...
websocket_info({'DOWN', _, process, Pid, _}, State = #state{request_guild_members_pid = WorkerPid}) when
    Pid =:= WorkerPid ->
    {ok, State#state{request_guild_members_pid = undefined}};
...
handle_gateway_payload(
    ...
    close_with_reason(not_authenticated, <<"Not authenticated">>, State);
handle_gateway_payload(
    request_guild_members, #{<<"d">> := _Data}, State = #state{session_pid = Pid, request_guild_members_pid = ExistingPid}
) when
    is_pid(Pid), ExistingPid =/= undefined
->
    {ok, State};
handle_gateway_payload(
...
->
    ...
    {ok, State#state{request_guild_members_pid = WorkerPid}};
handle_gateway_payload(
...
check_rate_limit(State = #state{rate_limit_state = RateLimitState}, Op) ->
    Now = erlang:system_time(millisecond),
    ...
    case EventsCount >= ?RATE_LIMIT_MAX_EVENTS of
        true ->
            rate_limited;
        false ->
            ...
            NewState = State#state{rate_limit_state = NewRateLimitState},
            check_opcode_rate_limit(Op, Now, NewState)
    end.

check_opcode_rate_limit(request_guild_members, Now, State = #state{rate_limit_state = RateLimitState}) ->
    OpcodeEvents = maps:get(request_guild_members_events, RateLimitState, []),
    FilteredEvents = [T || T <- OpcodeEvents, (Now - T) < ?REQUEST_GUILD_MEMBERS_RATE_LIMIT_WINDOW_MS],
    case length(FilteredEvents) >= ?REQUEST_GUILD_MEMBERS_RATE_LIMIT_MAX_EVENTS of
        true ->
            rate_limited;
        false ->
            NewOpcodeEvents = [Now | FilteredEvents],
            NewRateLimitState = maps:put(request_guild_members_events, NewOpcodeEvents, RateLimitState),
            {ok, State#state{rate_limit_state = NewRateLimitState}}
    end;
check_opcode_rate_limit(_OpAtom, _Now, State) ->
    {ok, State}.
