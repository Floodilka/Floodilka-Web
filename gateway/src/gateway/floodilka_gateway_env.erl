...
-module(floodilka_gateway_env).

-export([load/0, get/1, get_optional/1, get_map/0, patch/1, update/1]).

-define(APP, floodilka_gateway).
-define(CONFIG_TERM_KEY, {floodilka_gateway, runtime_config}).
...
build_config() ->
    #{
        ws_port => env_int("FLOODILKA_GATEWAY_WS_PORT", ws_port, 8080),
        rpc_port => env_int("FLOODILKA_GATEWAY_RPC_PORT", rpc_port, 8081),
        ...
        identify_rate_limit_enabled => env_bool("FLOODILKA_GATEWAY_IDENTIFY_RATE_LIMIT_ENABLED", identify_rate_limit_enabled, false),
        push_enabled => env_bool("FLOODILKA_GATEWAY_PUSH_ENABLED", push_enabled, true),
        push_user_guild_settings_cache_mb => env_int("FLOODILKA_GATEWAY_PUSH_USER_GUILD_SETTINGS_CACHE_MB",
                                                    push_user_guild_settings_cache_mb, 1024),
        push_subscriptions_cache_mb => env_int("FLOODILKA_GATEWAY_PUSH_SUBSCRIPTIONS_CACHE_MB",
                                              push_subscriptions_cache_mb, 1024),
        push_blocked_ids_cache_mb => env_int("FLOODILKA_GATEWAY_PUSH_BLOCKED_IDS_CACHE_MB",
                                             push_blocked_ids_cache_mb, 1024),
        presence_cache_shards => env_optional_int("FLOODILKA_GATEWAY_PRESENCE_CACHE_SHARDS", presence_cache_shards),
        presence_bus_shards => env_optional_int("FLOODILKA_GATEWAY_PRESENCE_BUS_SHARDS", presence_bus_shards),
        presence_shards => env_optional_int("FLOODILKA_GATEWAY_PRESENCE_SHARDS", presence_shards),
        guild_shards => env_optional_int("FLOODILKA_GATEWAY_GUILD_SHARDS", guild_shards),
        metrics_host => env_optional_string("FLOODILKA_METRICS_HOST", metrics_host),
        ...
        vapid_email => env_binary("VAPID_EMAIL", vapid_email, <<"support@floodilka.com">>),
